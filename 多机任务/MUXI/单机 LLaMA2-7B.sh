...
if [ ${MODEL_SIZE} == 7 ]; then 
    HIDDEN_SIZE=4096; NUM_HEAD=32; NUM_QUERY_GROUP=32; NUM_LAYERS=32; FFN_HIDDEN_SIZE=11008; NORM_EPS=1e-5;
else 
    echo "invalid MODEL_SIZE: ${MODEL_SIZE}"; exit 1
fi
...
LOG_NAME=llama2-7b_pretrain_WS${NNODES}_TP${TP}_PP${PP}_RoCE
...
CMD="${LAUNCHER} \
       ...
       ${DATA_ARGS} \
       "
echo "Environment Check: MCCL_IB_HCA=${MCCL_IB_HCA}, GID=${MCCL_IB_GID_INDEX}"
echo "Checking libmccl.so location:"
ldd /usr/bin/python3 | grep mccl || echo "MCCL lib not linked to python directly (normal for PyTorch)"
find ${MACA_PATH}/lib -name "libmccl*"
